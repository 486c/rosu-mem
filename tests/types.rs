...
#[test]
fn test_string_from_ptr_with_limit_randomized() {
    let mut rng = thread_rng();
    ...
    for _ in 0..=1337 {
        ...
        let read_string_res =
            p.read_string_with_limit_from_ptr(0, STRING_LIMIT);
        if string_len as usize > STRING_LIMIT {
            assert!(read_string_res.is_err());
            assert!(matches!(
                read_string_res,
                Err(ProcessError::StringTooLarge)
            ));
        } else {
            let read_string = read_string_res.unwrap();
            assert_eq!(read_string, random_string);
        }
    }
}
